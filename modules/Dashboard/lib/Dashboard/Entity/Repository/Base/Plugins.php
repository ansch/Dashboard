 ...
 *
 * This is the base repository class for plugins entities.
 */
class Dashboard_Entity_Repository_Base_Plugins extends EntityRepository
{
    /**
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE Dashboard_Entity_Plugins');
        $query->execute();
    }
     ...
     *
     * @return array|Dashboard_Entity_Plugins retrieved data array or Dashboard_Entity_Plugins instance
     */
     ...
     * @param boolean $useJoins Whether to include joining related objects (optional) (default=true).
     *
     * @return ArrayCollection collection containing retrieved Dashboard_Entity_Plugins instances
     */
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub = '';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.active = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.name LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.title LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.dbsize = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.dbfile LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.ajax = \'' . $fragment . '\'';
    ...
    public function selectCount($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numPlugins';
        if ($useJoins === true) {
        ...
        $qb->select($selection)
           ->from('Dashboard_Entity_Plugins', 'tbl');
     ...
     * @param int    $excludeid  Id of plugins to exclude (optional).
...
// TODO - see https://github.com/zikula/core/issues/118
        ...
        //$fu = new FilterUtil('Dashboard', $this);
