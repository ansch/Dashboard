...
class Dashboard_Api_User extends Dashboard_Api_Base_User
{
    ...
    public function cleanOrder($args)
        {
                ...
                $objectType = 'boxx';
                $entityClass = 'Dashboard_Entity_' . ucfirst($objectType);
                $repository = $this->entityManager->getRepository($entityClass)->findBy(array('userid' => $userid, 'block' => $block), array('boxorder' => ASC, 'id' => DESC));
                $where = "tbl.userid = $userid AND tbl.block=$block";             
                $order = 'tbl.boxorder ASC, tbl.id DESC';
                $objects = $repository;
    ...
    public function getBoxes($args)
        {
            ...
            $dql = "SELECT id FROM Dashboard_Entity_Boxx id" .
                " WHERE id.userid = $userid ORDER BY id.boxorder ASC, id.id DESC";     
            $em = ServiceUtil::getService('doctrine.entitymanager');
            $query = $em->createQuery($dql);
            $boxes = $query->getArrayResult();
        
            //$boxes = Doctrine_Core::getTable('Dashboard_Model_Boxx')->findByDql($dql)->toArray();
            ...
            if (!$boxes) {
                    ...
                    foreach ($boxes as $box) {
                            ...
                            if (is_array($plugin)) {
                                    Loader::requireOnce($plugin['pluginfile']);
                            $class=$plugin['name'];
                            $cplugin = new $class();
                        $box['title']   = $cplugin->title();
                        $box['boxsize'] = $cplugin->size();
                        ...
                        $box['width'] 		= 32*(int)$box['boxsize'];
                        $next[$previous] 	= $box['id'];
                        $previous 			= $box['id'];
                        if ($size + $box['boxsize'] <= 3) {
                            $box['newline'] = false;
                        } else {
                            $box['newline'] = true;
                            $size = 0;
    ...
    */            
    ...
    public function getPlugin($args)
        ...
        // Return the item array
        $entity = $this->name . '_Entity_Plugin';
                ...
                if ($plugin['active'] == -1 || $plugin['active'] == 0) {
                    ...
                    foreach ($boxes as $box) {
                                if ($box['plugin'] == $plugin['name']) {
                                        $found = true;
                                } elseif ($onlySingle && $plugin['boxsize'] != 1) {
                                        $found = true;
                                }
                        }
                    if (!$found) {
    ...
    public function delete($args)
        {
            ...
            // Get Object
            $obj = DBUtil::selectObjectByID('dashboard_boxx',$id);
            if (!$obj) {
                    return false;
            } else {
