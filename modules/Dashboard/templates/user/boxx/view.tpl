...
<div class="z-frontendcontainer">
    ...
    {checkpermissionblock component='Dashboard::' instance='.*' level="ACCESS_ADD"}
        ...
        <a href="{modurl modname='Dashboard' type='user' func='edit' ot='boxx'}" title="{$createTitle}" class="z-icon-es-add">
            {$createTitle}
        </a>
    ...
    {if isset($showAllEntries) && $showAllEntries eq 1}
        ...
        <a href="{modurl modname='Dashboard' type='user' func='view' ot='boxx'}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
        {assign var='all' value=1}
    {else}
        {gt text='Show all entries' assign='linkTitle'}
        <a href="{modurl modname='Dashboard' type='user' func='view' ot='boxx' all=1}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
    {/if}
...
<table class="z-datatable">
    ...
    <tr>
        <th id="huserid" scope="col" class="z-right">
            {sortlink __linktext='Userid' sort='userid' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='boxx'}
        </th>
        <th id="hboxorder" scope="col" class="z-right">
            {sortlink __linktext='Boxorder' sort='boxorder' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='boxx'}
        </th>
        <th id="hblock" scope="col" class="z-right">
            {sortlink __linktext='Block' sort='block' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='boxx'}
        </th>
        <th id="hplugin" scope="col" class="z-left">
            {sortlink __linktext='Plugin' sort='plugin' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='boxx'}
        </th>
        <th id="hpage" scope="col" class="z-right">
            {sortlink __linktext='Page' sort='page' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='boxx'}
        </th>
        <th id="hboxdata" scope="col" class="z-left">
            {sortlink __linktext='Boxdata' sort='boxdata' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='boxx'}
        </th>
