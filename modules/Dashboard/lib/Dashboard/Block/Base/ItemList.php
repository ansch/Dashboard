 ...
 */
class Dashboard_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('Dashboard:ItemListBlock:', 'Block title::');
    }
    ...
    public function info()
    {
        ...
        if (!ModUtil::available('Dashboard')) {
            $requirementMessage .= $this->__('Notice: This block will not be displayed until you activate the Dashboard module.');
        }

        return array('module'          => 'Dashboard',
            'text_type'       => $this->__('Item list'),
            'text_type_long'  => $this->__('Show a list of Dashboard items based on different criteria.'),
            'allow_multiple'  => true,
            'form_content'    => false,
            'form_refresh'    => false,
            'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('Dashboard:ItemListBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        ModUtil::initOOModule('Dashboard');

        if (!isset($vars['objectType']) || !in_array($vars['objectType'], Dashboard_Util_Controller::getObjectTypes('block'))) {
            $vars['objectType'] = Dashboard_Util_Controller::getDefaultObjectType('block');
        }
        ...
        $repository = $entityManager->getRepository('Dashboard_Entity_' . ucfirst($objectType));

        $idFields = ModUtil::apiFunc('Dashboard', 'selection', 'getIdFields', array('ot' => $objectType));
        ...
        list($entities, $objectCount) = ModUtil::apiFunc('Dashboard', 'selection', 'getEntitiesPaginated', $selectionArgs);
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('Dashboard items');
        }
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->getPost()->filter('objecttype', 'boxes', FILTER_SANITIZE_STRING);
        ...
        if (!in_array($vars['objectType'], Dashboard_Util_Controller::getObjectTypes('block'))) {
            $vars['objectType'] = Dashboard_Util_Controller::getDefaultObjectType('block');
        }
