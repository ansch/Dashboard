 ...
 *
 * This is the base repository class for boxx entities.
 */
class Dashboard_Entity_Repository_Base_Boxx extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'userid';
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE Dashboard_Entity_Boxx');
        $query->execute();
    }
     ...
     *
     * @return array|Dashboard_Entity_Boxx retrieved data array or Dashboard_Entity_Boxx instance
     */
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub = '';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.userid = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.boxorder = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.block = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.plugin LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.page = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.boxdata LIKE \'%' . $fragment . '%\'';
    ...
    public function selectCount($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numBoxes';
        if ($useJoins === true) {
        ...
        $qb->select($selection)
           ->from('Dashboard_Entity_Boxx', 'tbl');
     ...
     * @param int    $excludeid  Id of boxes to exclude (optional).
