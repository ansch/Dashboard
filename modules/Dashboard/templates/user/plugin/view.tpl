...
<div class="z-frontendcontainer">
    ...
    {checkpermissionblock component='Dashboard::' instance='.*' level="ACCESS_ADD"}
        ...
        <a href="{modurl modname='Dashboard' type='user' func='edit' ot='plugin'}" title="{$createTitle}" class="z-icon-es-add">
            {$createTitle}
        </a>
    ...
    {if isset($showAllEntries) && $showAllEntries eq 1}
        ...
        <a href="{modurl modname='Dashboard' type='user' func='view' ot='plugin'}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
        {assign var='all' value=1}
    {else}
        {gt text='Show all entries' assign='linkTitle'}
        <a href="{modurl modname='Dashboard' type='user' func='view' ot='plugin' all=1}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
    {/if}
...
<table class="z-datatable">
    ...
    <tr>
        <th id="hactive" scope="col" class="z-right">
            {sortlink __linktext='Active' sort='active' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='plugin'}
        </th>
        <th id="hname" scope="col" class="z-left">
            {sortlink __linktext='Name' sort='name' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='plugin'}
        </th>
        <th id="htitle" scope="col" class="z-left">
            {sortlink __linktext='Title' sort='title' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='plugin'}
        </th>
        <th id="hboxsize" scope="col" class="z-right">
            {sortlink __linktext='Boxsize' sort='boxsize' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='plugin'}
        </th>
        <th id="hpluginfile" scope="col" class="z-left">
            {sortlink __linktext='Pluginfile' sort='pluginfile' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='plugin'}
        </th>
        <th id="hajax" scope="col" class="z-right">
            {sortlink __linktext='Ajax' sort='ajax' currentsort=$sort sortdir=$sdir all=$all modname='Dashboard' type='user' func='view' ot='plugin'}
        </th>
